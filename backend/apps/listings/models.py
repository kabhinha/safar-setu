...
class Hotspot(models.Model):
    ...
    # Decision-at-Time (Kiosk Discovery)
    class HotspotType(models.TextChoices):
        Standard = 'STANDARD', 'Standard Hotspot'
        Sight = 'SIGHT', 'Sight'

    class SightsCategory(models.TextChoices):
    ...
    class DistanceBand(models.TextChoices):
        Near = 'NEAR', 'Near'
        Medium = 'MEDIUM', 'Medium'
        Far = 'FAR', 'Far'

    class TransportHubType(models.TextChoices):
        Bus = 'BUS_STAND', 'Bus Stand'
        Rail = 'RAILWAY_STATION', 'Railway Station'
        Taxi = 'TAXI_POINT', 'Taxi Point'
        Ferry = 'FERRY', 'Ferry'
        Other = 'OTHER', 'Other'

    approx_travel_time_min = models.IntegerField(null=True, blank=True, help_text="Approx travel time in minutes")
    distance_band = models.CharField(max_length=20, choices=DistanceBand.choices, default=DistanceBand.Medium)
    nearest_transport_hub_name = models.CharField(max_length=150, blank=True, help_text="Name of nearest hub")
    nearest_transport_hub_type = models.CharField(max_length=20, choices=TransportHubType.choices, blank=True)

    # Sights & Classification
    hotspot_type = models.CharField(max_length=20, choices=HotspotType.choices, default=HotspotType.Standard)
    sights_category = models.CharField(max_length=20, choices=SightsCategory.choices, blank=True, null=True)
